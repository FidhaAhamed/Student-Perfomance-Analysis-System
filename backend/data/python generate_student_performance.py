...
def determine_performance(attendance, assignments, tutorials, volunteering, moocs, extraCurricularActivities):
    if attendance >= 75:
        if assignments >= 7:
            if tutorials >= 3:
                if volunteering >= 2:
...
for i in range(1000):
    ...
    name = random_name()
    attendance = random.randint(50, 100)  # 50-100%
    assignments = random.randint(3, 10)
    tutorials = random.randint(0, 5)
    volunteering = random.randint(0, 5)
    moocs = random.randint(0, 3)
    extraCurricularActivities = random.randint(0, 5)
    performance = determine_performance(attendance, assignments, tutorials, volunteering, moocs, extraCurricularActivities)
    rows.append([
        student_id, name, attendance, assignments, tutorials, volunteering, moocs, extraCurricularActivities, performance
    ])

with open('student_performance.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['StudentID', 'Name', 'Attendance', 'Assignments', 'Tutorials', 'Volunteering', 'MOOCs', 'ExtraCurricularActivities', 'PerformanceLevel'])
